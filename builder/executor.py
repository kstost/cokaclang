...
"""
    ...
    def _find_binary(self, target: Target) -> Optional[Path]:
        ...
        binary_name = "cokaclang.exe" if target.platform == "windows" else "cokaclang"
    ...
    def copy_to_dist(self, results: List[BuildResult]) -> List[Tuple[Path, str]]:
        ...
        for result in results:
            ...
            if result.target.platform == "windows":
                dest_name = f"cokaclang-{result.target.friendly_name}.exe"
            else:
                dest_name = f"cokaclang-{result.target.friendly_name}"
